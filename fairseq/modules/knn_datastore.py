...
class KNN_Dstore(object):
    ...
    def setup_faiss(self, args):
        ...
        if args.move_dstore_to_mem:
            ...
            if self.use_gpu_to_search:
                self.keys = torch.from_numpy(self.vals)
                if torch.cuda.is_available():
    ...
    def retrieve(self, queries):
        ...
        dists, indexs = self.get_knns(queries.contiguous().view(-1, queries.size(-1)))  # [Batch * seq len, K]
        ...
        tgt_index = self.keys[indexs].to(queries.device).squeeze(-1)
